/**
 * Ability Definitions for Fantasy Autobattler.
 ...
 * 
 * @description The Knight raises their shield, significantly
 * reducing incoming physical damage for a short duration.
 */
 ...
 * 
 * @description The Guardian draws enemy attention,
 * protecting allies by forcing enemies to target them.
 */
 ...
 * 
 * @description The Berserker becomes more dangerous as they
 * take damage, entering a rage state at low health.
 */
const rage: PassiveAbility = {
  ...
  targetType: 'self',
  trigger: 'on_low_hp',
  triggerThreshold: 50, // Triggers at 50% HP
  range: 0,
  effects: [
    {
      type: 'buff',
      stat: 'attack',
      percentage: 0.5, // +50% attack
      duration: 999, // Lasts until HP goes above threshold
      stackable: false,
    },
  ],
  icon: 'rage',
 ...
 * 
 * @description The Rogue excels at flanking maneuvers,
 * dealing devastating damage to enemies facing away.
 */
const backstab: PassiveAbility = {
  ...
  effects: [
    {
      type: 'damage',
      value: 0, // Bonus damage calculated from base attack
      damageType: 'physical',
      attackScaling: 1.0, // +100% of attack as bonus damage
    },
  ],
  icon: 'dagger',
};
 ...
 * 
 * @description The Duelist's expert swordsmanship allows them
 * to parry and immediately strike back at attackers.
 */
  ...
  effects: [
    {
      ...
      value: 0,
      damageType: 'physical',
      attackScaling: 0.75, // 75% of attack as counter damage
 ...
 * 
 * @description The Assassin specializes in finishing off
 * wounded targets with lethal precision.
 */
const execute: PassiveAbility = {
  ...
  trigger: 'on_attack',
  triggerThreshold: 30, // Target must be below 30% HP
  range: 1,
  effects: [
    {
      type: 'damage',
      value: 0,
      damageType: 'physical',
      attackScaling: 1.0, // +100% damage to low HP targets
    },
  ],
  icon: 'skull',
};
 ...
 * 
 * @description The Archer unleashes a rain of arrows,
 * hitting multiple enemies in a targeted area.
 */
const volley: ActiveAbility = {
  ...
  effects: [
    {
      type: 'damage',
      value: 12, // Base damage per target
      damageType: 'physical',
      attackScaling: 0.5, // +50% of attack
    },
  ],
  icon: 'arrows',
};
 ...
 * 
 * @description The Crossbowman's heavy bolt punches through
 * armor, dealing significant damage to armored targets.
 */
const piercingShot: ActiveAbility = {
  ...
  effects: [
    {
      type: 'damage',
      value: 25, // High base damage
      damageType: 'physical',
      attackScaling: 0.8, // +80% of attack
    },
    {
      type: 'debuff',
      stat: 'armor',
      percentage: 0.5, // -50% armor for damage calculation
      duration: 0, // Instant effect (only for this attack)
    },
  ],
  icon: 'crossbow',
};
 ...
 * 
 * @description The Hunter sets a cunning trap that
 * immobilizes enemies who trigger it.
 */
 ...
 * 
 * @description The Mage hurls a ball of fire that explodes
 * on impact, damaging all enemies in the blast radius.
 */
const fireball: ActiveAbility = {
  ...
  effects: [
    {
      type: 'damage',
      value: 30, // High magic damage
      damageType: 'magical', // Ignores armor
      attackScaling: 0.6, // +60% of attack
    },
  ],
  icon: 'fireball',
};
 ...
 * 
 * @description The Warlock drains the life force from enemies,
 * using dark magic to sustain themselves in battle.
 */
  ...
  effects: [
    {
      ...
      value: 20,
      damageType: 'magical',
      attackScaling: 0.7, // +70% of attack
    },
    {
      type: 'heal',
      value: 0, // Calculated as 50% of damage dealt
      attackScaling: 0.35, // 50% of damage (0.7 * 0.5)
    },
  ],
  icon: 'drain',
};
 ...
 * 
 * @description The Elementalist calls down lightning that
 * arcs between enemies, dealing reduced damage with each jump.
 */
const chainLightning: ActiveAbility = {
  ...
  effects: [
    {
      type: 'damage',
      value: 35, // Primary target damage
      damageType: 'magical',
      attackScaling: 0.8, // +80% of attack
    },
    // Note: Chain effect handled in battle logic
    // Secondary targets take 70% damage, tertiary 50%
  ],
  icon: 'lightning',
};
 ...
 * 
 * @description The Priest channels divine energy to
 * mend the wounds of their allies.
 */
const heal: ActiveAbility = {
  ...
  effects: [
    {
      type: 'heal',
      value: 25, // Base heal amount
      attackScaling: 0.5, // +50% of attack as bonus healing
      canOverheal: false,
    },
  ],
  icon: 'heal',
 ...
 * 
 * @description The Bard's inspiring music bolsters
 * the fighting spirit of all allied units.
 */
const inspire: ActiveAbility = {
  ...
  cooldown: 4,
  range: 0, // Affects all allies regardless of position
  effects: [
    {
      type: 'buff',
      stat: 'attack',
      percentage: 0.2, // +20% attack
      duration: 2,
      stackable: false,
    },
  ],
 ...
 * 
 * @description The Enchanter weaves a spell that
 * temporarily incapacitates an enemy.
 */
 ...
 * Used for abilities that reduce cooldowns.
 * 
 ...
 * const percentReduced = calculateCooldownReduction(4, 0.25, true); // 3
 */
 ...
 * Replaces placeholders with actual effect values.
 * 
 ...
 * const ready = isOnCooldown(0); // false
 */
...
export function isInAbilityRange(
  ...
  // Self-targeting abilities always succeed
  if (ability.targetType === 'self') {
    return true;
  }
  
  // All allies/enemies abilities have unlimited range
  if (
 ...
 * @example
 * const hasActive = unitHasActiveAbility('mage'); // true (fireball)
 * const hasPassive = unitHasActiveAbility('rogue'); // false (backstab is passive)
 */
 ...
 * @example
 * const hasPassive = unitHasPassiveAbility('rogue'); // true (backstab)
 * const hasActive = unitHasPassiveAbility('mage'); // false (fireball is active)
 */
