...
import { Position, BattleUnit } from '../types/game.types';
import { manhattanDistance, isValidPosition, getNeighbors, positionToKey, Grid } from './grid';
import { PATHFINDING_CONSTANTS } from '../config/game.constants';
...
export function findPath(
  ...
  if (!isWalkableForPathfinding(goal, grid, units, movingUnit)) {
    return [];
  }
  ...
  // Main A* loop
  while (!openSet.isEmpty() && iterations < PATHFINDING_CONSTANTS.MAX_ITERATIONS) {
    iterations++;
    ...
    for (const neighborPos of neighbors) {
      ...
      if (!isWalkableForPathfinding(neighborPos, grid, units, movingUnit)) {
        continue;
      }

      // Calculate costs
      const tentativeGCost = current.gCost + PATHFINDING_CONSTANTS.MOVEMENT_COST;
      ...
      const neighborNode = createPathNode(
        neighborPos,
        tentativeGCost,
        hCost,
        current
...
export function findPathWithMaxLength(
  ...
  const fullPath = findPath(start, goal, grid, units, movingUnit);
    ...
    for (let dy = -range; dy <= range; dy++) {
      for (let dx = -range; dx <= range; dx++) {
        ...
        if (!isValidPosition(candidate) || 
            !isWalkableForPathfinding(candidate, grid, units, movingUnit)) {
          continue;
        }

        // Check if we can reach this position
        const path = findPath(start, candidate, grid, units, movingUnit);
...
export function hasPath(
  ...
  const path = findPath(start, goal, grid, units, movingUnit);
