...
import { getUnitTemplate } from '../units/unit.data';
...
describe('BotGenerator', () => {
  ...
  describe('generateBotPositions', () => {
    it('should place tanks and melee in front line', () => {
      ...
      if (knight && rogue && archer && mage) {
        ...
        expect(knightPos?.y).toBe(DEPLOYMENT_ZONES.ENEMY_ROWS[1]);
        expect(roguePos?.y).toBe(DEPLOYMENT_ZONES.ENEMY_ROWS[1]);
        
        const archerPos = positions[2];
        const magePos = positions[3];
        
        expect(archerPos?.y).toBe(DEPLOYMENT_ZONES.ENEMY_ROWS[0]);
        expect(magePos?.y).toBe(DEPLOYMENT_ZONES.ENEMY_ROWS[0]);
      }
    });
    ...
    it('should handle single unit', () => {
      ...
      if (knight) {
        ...
        expect(positions[0]?.y).toBe(DEPLOYMENT_ZONES.ENEMY_ROWS[1]);
      }
    });
    ...
    it('should not exceed grid boundaries', () => {
      ...
      if (knight) {
        const manyUnits = Array(20).fill(knight);
  ...
  describe('validateBotTeam', () => {
    ...
    it('should reject oversized team', () => {
      ...
      if (knight) {
        const oversizedTeam = Array(10).fill(knight);
