...
/**
 * Result of applying a mechanic, containing updated state and generated events.
 * Mechanics can optionally generate events for battle log display.
 *
 * @example
 * const result: MechanicResult = {
 *   state: updatedState,
 *   events: [{ type: 'mechanic_armor_shred', ... }],
 * };
 */
export interface MechanicResult {
  /** Updated battle state after mechanic application */
  state: BattleState;
  /** Events generated by this mechanic (for battle log) */
  events?: BattleEvent[];
}
...
export interface MechanicProcessor {
   ...
   *          Can also return MechanicResult with events for battle log
   */
  apply(
    phase: BattlePhase,
    state: BattleState,
...
/**
 * Result of processing all mechanics for a phase.
 * Contains updated state and all events generated by mechanics.
 *
 * @example
 * const result = processor.process('attack', state, context);
 * console.log(`Generated ${result.events.length} mechanic events`);
 */
export interface ProcessResult {
  /** Updated battle state after all mechanics applied */
  state: BattleState;
  /** All events generated by mechanics during this phase */
  events: BattleEvent[];
}
 ...
 *
 * // Process a phase
 * const result = processor.process('pre_attack', state, {
 *   activeUnit: attacker,
 *   target: defender,
 ...
 * // result.events - mechanic events for battle log
 */
export interface MechanicsProcessor {
   ...
   * @param state - Current battle state
   * @param context - Phase context with active unit and action info
   * @returns ProcessResult with updated state and generated events
   */
  process(
    phase: BattlePhase,
    state: BattleState,
...
/**
 * Checks if a mechanic result is a MechanicResult object (with events).
 *
 * @param result - Result from mechanic processor
 * @returns True if result is MechanicResult with state property
 */
 ...
 * Collects events from all mechanics for battle log.
 *
 * @param phase - Current battle phase
 * @param state - Current battle state
 ...
 * @returns ProcessResult with updated state and collected events
 */
export function applyMechanics(
  ...
  for (const mechanic of mechanicsForPhase) {
    ...
    if (processor) {
      ...
      // Handle both BattleState and MechanicResult returns
      if (isMechanicResult(mechanicResult)) {
        result = mechanicResult.state;
        if (mechanicResult.events && mechanicResult.events.length > 0) {
