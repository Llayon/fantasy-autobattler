...
/**
 * Union type for all intercept-related events (internal to processor).
 * Note: This is different from InterceptEvent in event.types.ts
 * which is used for battle event logging.
 */
export type InterceptProcessorEvent =
  | HardInterceptEvent
  | SoftInterceptEvent
...
export interface InterceptFullResult {
  ...
  events: InterceptProcessorEvent[];
